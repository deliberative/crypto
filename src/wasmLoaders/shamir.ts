...
import shamirMethods from "../../dist/shamirMethods";

const loadShamir = async (memoryLen: number) => {
  ...
  let buf: Uint8Array;
  if (isBrowser()) {
    const raw = window.atob(shamirMethods);
    ...
    buf = Buffer.from(shamirMethods, "base64");
  }
